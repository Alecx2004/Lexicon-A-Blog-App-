...
function Home() {
  ...
  useEffect(() => {
    // Navigate after posts have been fetched
    if (posts.length > 0) {
      navigate("/all-posts");
    }
  }, [posts, navigate]);
          ...
          <div className="flex flex-wrap">
            <div className="p-2 w-full flex flex-col md:flex-row justify-around items-center gap-8">
              <div className={`max-w-2xl ${styles.fadeInRight}`}>
                ...
                <h5
                  className={`text-xl font-medium text-sky-600 ${styles.fadeInRight} ${styles.delaySmall}`}
                >
                  Fuel Your Curiosity, Enrich Your Mind, Elevate Your Day
                </h5>
                <Link to="/signup">
                  <Button
                    className={`mt-8 md:mt-20 text-xl ${styles.fadeInUp} ${styles.delayMedium}`}
                  >
                    Get Started
                  </Button>
                </Link>
              ...
              <div
                className={`max-w-lg ${styles.fadeInLeft} ${styles.delaySmall}`}
              >
                <img src={Image} alt="Blog Landing" className="w-full h-auto" />
              </div>
            </div>
          </div>
